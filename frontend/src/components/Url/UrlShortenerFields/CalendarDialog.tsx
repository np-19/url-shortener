...
const CalendarDialog = ({ panelRef, value, onChange, onClose }: CalendarDialogProps) => {
  return createPortal(
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-silver-950/15 px-3 py-5 backdrop-blur-[2px] sm:py-8">
      <button type="button" className="absolute inset-0 cursor-default" aria-label="Close calendar overlay" onClick={onClose} />
      <div ref={panelRef} className="w-full max-w-[19rem] sm:max-w-[21rem]">
